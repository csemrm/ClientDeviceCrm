...
//Rare exceptions to indicate a bug in the _saratogacrm code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_saratogacrm = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_saratogacrm]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.saratogacrm.module"
