...
NSString * SaratogaCRM$ModuleRequireFormat = @"(function(exports){"
...
@implementation SaratogaCRMObject
...
{
	// create SaratogaCRM global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "SaratogaCRM" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_saratogacrm = [[SaratogaCRMObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_saratogacrm];
	
	NSString *_saratogacrmNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _saratogacrmNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_saratogacrm valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:SaratogaCRM$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.saratogacrm.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.saratogacrm.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
